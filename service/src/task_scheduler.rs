...
pub struct TaskSchedulerServiceImpl {
    repo: Arc<dyn ITaskRepository + Sync + Send>,
    sub_repo: Arc<dyn ISubTaskRepository + Sync + Send>,
    report_service: Arc<dyn TaskReportService + Sync + Send>,
    sub_task_services: HashMap<TaskDisplayType, Arc<dyn SubTaskService + Sync + Send>>,
    max_tasks_count: u32,
    #[builder(default)]
